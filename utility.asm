...
start:      ldi   0d000h.1              ; get msb of address to copy from
            phi   r7
...
indigit:    sep   ra                    ; get keystroke
            ...
            sdi   '5'                   ; if greater than 5 try again
            lbnf  indigit

            sdi   '5'-'1'               ; if less than 1 try again
            lbnf  indigit
...
inbacks:    ldi   msgback.1             ; pointer to backspace sequence
            ...
            ; Enter installer by jumping to 9013, but since it uses BIOS
            ; initialze SCALL and console first.

install:    ldi   msgline.1             ; get blank line sequence
            phi   r7
            ldi   msgline.0
            plo   r7

            sep   rc                    ; output blank line
            dec   r2

            ldi   goinst.1             ; return address for f_initcall
            phi   r6
            ldi   goinst.0
            plo   r6
            ...
            lbr   f_initcall            ; initialize r4, r5 for scall

goinst:     sep   scall                 ; intialize console
            dw    f_setbd

            lbr   9013h                 ; jump to installer entry point


            ; Enter SEDIT by jumping to D809, but since it uses BIOS
            ; initialze SCALL and console first.

sedit:      ldi   msgline.1             ; get blank line sequence
            phi   r7
            ldi   msgline.0
            plo   r7
            ...
            ldi   gosedit.1             ; return address for f_initcall
            phi   r6
            ldi   gosedit.0
            plo   r6
...
gosedit:    sep   scall                 ; intialize console
            dw    f_setbd

            lbr   0d809h                ; jump to sedit entry point


            ; Boot system. Do a full initialization of stack, SCALL, and
            ; console first as this might be called directly.

boot:       plo   rf                    ; enough for 300 baud at 10 Mhz
            ldi   21
...
bootdly:    phi   rf
            ...
            lbr   0f600h                ; boot system via bios
...
msgimen:    db    cr,lf,cr,lf
            db    "Firmware Menu for MBIOS VERSION",cr,lf

            db    cr,lf
            db    "1. Install Elf/OS",cr,lf
            db    "2. Run SEDIT",cr,lf
            db    "3. Boot Elf/OS",cr,lf
            db    "4. EEPROM Utility",cr,lf,cr,lf
            db    "   Option ? ",0

msgeban:    db   cr,lf,cr,lf
            db   "EEPROM Utility for MBIOS VERSION",cr,lf,0
